...
2. **Step 1+2 — Presidio analyze**: `AnalyzerEngine` with custom recognizers from `recognizers.get_all_custom_recognizers()` (regex-based TW recognizers + mandatory CKIP NER). Supported entity types are the keys of `TOKEN_MAP` in `config.py`. `BRANCH` is dropped from the request list unless `mask_branch_code=True`.
...
- `conflict_resolver.py` v4 Step 0: exact-span dedup is the only code path that removes CKIP × spaCy × AddressEnhanced duplicate PERSON/LOCATION detections — do not bypass it.
